...
export const PYTH_IDS: Record<string, string> = ASSETS_CATALOG.reduce((acc, asset) => {
  acc[asset.key] = asset.pythFeedId;
  return acc;
...
export async function computeLean(
  ...
  if (!series) {
    return { ok: false, asset, decision: 'SKIP', phase: 'unknown', message: 'unknown_asset' };
  }
  ...
  if (liveSpot) {
    ...
    if (!feedId) return { ok: false, asset, decision: 'SKIP', phase: pick.phase, message: 'no_pyth_id' };
