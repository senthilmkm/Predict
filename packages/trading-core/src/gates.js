...
exports.evaluateStaticGate = evaluateStaticGate;
function money2(n) {
    return (Math.round(n * 10000) / 10000).toFixed(4);
}
function evaluateStaticGate(lean, cfg, opts) {
    ...
    let ask = 0.9;
    if (lean.decision === 'YES' && lean.yes_ask != null)
        ask = Number(lean.yes_ask);
    if (lean.decision === 'NO' && lean.no_ask != null)
        ask = Number(lean.no_ask);
    ...
    const dollars = Math.min(cfg.risk.fixed_dollars_per_trade, cfg.risk.max_dollars_per_trade);
    ...
    const maxDollars = Math.min(cfg.risk.max_dollars_per_trade, cfg.risk.fixed_dollars_per_trade);
    ...
    const side = lean.decision === 'YES' ? 'bid' : 'ask';
