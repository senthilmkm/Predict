...
describe('evaluateStaticGate edge cases', () => {
  ...
  test('Home Buy enter mult scales tap cushion without changing Cushion lean', () => {
    const cfg = base();
    cfg.cushions.Gold = 100;
    cfg.risk.home_enter_cushion_mult = 0.8;
    cfg.risk.cushion_lean_enter_mult = 1;
    expect(
      evaluateStaticGate(lean({ abs_gap: 80 }), cfg, { allowWhenAutoTradeOff: true }).ok
    ).toBe(true);
    expect(
      evaluateStaticGate(lean({ abs_gap: 80 }), cfg, { applyCushionLeanMaxGap: true }).skip_reason
    ).toBe('below_cushion');
    expect(
      evaluateStaticGate(lean({ abs_gap: 79 }), cfg, { allowWhenAutoTradeOff: true }).skip_reason
    ).toBe('below_cushion');
  });
