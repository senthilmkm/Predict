...
export interface RiskConfig {
   ...
   * Default 1. Range 0.5–1.5. Missing → 1. Home Buy ignores it.
   */
  cushion_lean_enter_mult?: number;
  /**
   * Cushion lean only. Skip buy when live gap ≥ cushion × this.
   * Default 2.5. Range 1.5–5. Missing → 2.5.
   */
  ...
  /** Final 60s + high ask: lead ≥ ATR ×. Missing → On. */
...
export function defaultAppConfig(): AppConfig {
  return {
    ...
    risk: {
      ...
      cheap_loop_assets: [],
      cheap_loop_hourly_enabled: false,
      cheap_loop_hourly_start_minutes: 10,
      cheap_loop_hourly_flatten_minutes: 5,
