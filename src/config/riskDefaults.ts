/**
 * Bundled risk defaults — mirrored from Command Center Kalshi / Predict tab.
 * Also seeded into phone storage (`foresight.risk.defaults.v3`) so
 * Settings → Restore defaults can reload them offline.
 */
import { AssetRegistry, RiskConfig, TimeInForce } from './types';

export const DEFAULT_RISK_CONFIG: RiskConfig = {
  fixed_dollars_per_trade: 5,
  max_dollars_per_trade: 5,
  min_dollars_per_trade: 1,
  max_open_positions: 5,
  max_trades_per_day: 100,
  max_trades_per_asset_per_window: 1,
  daily_loss_stop_usd: 50,
  min_minutes_left: 2,
  min_minutes_elapsed: 2,
  max_entry_ask_usd: 0.9,
  time_in_force: 'immediate_or_cancel',
  manual_buy_time_in_force: 'immediate_or_cancel',
  chase_above_ask_usd: 0.02,
  protect_sell_enabled: false,
  protect_sell_gap_ratio: 1,
  protect_sell_grace_seconds: 45,
  home_enter_cushion_mult: 1,
  home_buy_assets: AssetRegistry.getDefaultPathAssets(),
  home_sell_at_pct: 0,
  cushion_lean_sell_at_pct: 0,
  buffer_run_sell_at_pct: 0,
  cushion_lean_enabled: true,
  cushion_lean_assets: AssetRegistry.getDefaultPathAssets(),
  cushion_lean_enter_mult: 1,
  cushion_lean_max_gap_mult: 2.5,
  smart_buy_enabled: true,
  smart_buy_min_edge_usd: 0.08,
  cash_out_enabled: false,
  cash_out_fixed_dollars_per_trade: 5,
  cash_out_max_dollars_per_trade: 5,
  cash_out_min_dollars_per_trade: 1,
  cash_out_enter_pct: 60,
  cash_out_max_ask_usd: 0.82,
  cash_out_bid_usd: 0.88,
  cash_out_stop_usd: 0.05,
  cash_out_skip_thin_bid: false,
  cash_out_assets: ['Gold'],
  gold_fade_enabled: false,
  gold_fade_fixed_dollars_per_trade: 5,
  gold_fade_max_dollars_per_trade: 5,
  gold_fade_min_dollars_per_trade: 1,
  gold_fade_max_gap_usd: 3,
  gold_fade_max_ask_usd: 0.5,
  gold_fade_take_usd: 0.06,
  gold_fade_stop_usd: 0.05,
  gold_fade_flatten_minutes: 3,
  gold_fade_skip_thin_bid: false,
  twap_lock_enabled: false,
  twap_lock_fixed_dollars_per_trade: 5,
  twap_lock_max_dollars_per_trade: 5,
  twap_lock_min_dollars_per_trade: 1,
  twap_lock_assets: ['BTC', 'ETH'],
  twap_lock_max_ask_usd: 0.96,
  twap_lock_skip_thin_bid: false,
  last_minute_enabled: false,
  last_minute_side: 'yes',
  last_minute_max_ask_usd: 0.96,
  last_minute_watch_seconds: 150,
  last_minute_enter_seconds: 90,
  last_minute_stop_seconds: 10,
  last_minute_ladder_seconds: 2,
  last_minute_clip_count: 1,
  last_minute_max_clips: 5,
  last_minute_both_min_ask: 0.9,
  last_minute_both_gap: 0.1,
  last_minute_flip_sell_usd: 0,
  last_minute_atr_cushion_enabled: true,
  last_minute_atr_ask_usd: 0.88,
  last_minute_atr_mult: 1.25,
  last_minute_skip_thin_bid: false,
  last_minute_assets: AssetRegistry.getDefaultPathAssets(),
  step_buy_enabled: false,
  step_buy_start_minutes: 5,
  step_buy_cushion_pct: 50,
  step_buy_add_cushion_pct: 75,
  step_buy_sell_if_thesis_dies: true,
  step_buy_lot_count: 1,
  step_buy_add_wait_minutes: 1,
  step_buy_add_band_usd: 0.02,
  step_buy_max_lots: 3,
  step_buy_stop_usd: 0.03,
  step_buy_max_ask_usd: 0.8,
  step_buy_skip_thin_bid: false,
  step_buy_assets: AssetRegistry.getDefaultPathAssets(),
  spike_fade_enabled: false,
  spike_fade_start_minutes: 2,
  spike_fade_until_minutes: 6,
  spike_fade_expensive_min_usd: 0.75,
  spike_fade_expensive_max_usd: 0.8,
  spike_fade_cheap_min_usd: 0.2,
  spike_fade_cheap_max_usd: 0.25,
  spike_fade_take_ask_usd: 0.42,
  spike_fade_stop_ask_usd: 0.1,
  spike_fade_flatten_minutes: 3,
  spike_fade_lot_count: 1,
  spike_fade_skip_thin_bid: false,
  spike_fade_assets: AssetRegistry.getDefaultPathAssets(),
  pair_lock_enabled: false,
  pair_lock_start_minutes: 2,
  pair_lock_until_minutes: 10,
  pair_lock_runner_max_ask_usd: 0.6,
  pair_lock_min_lock_usd: 0.05,
  pair_lock_flatten_minutes: 3,
  pair_lock_runner_stop_usd: 0.1,
  pair_lock_lot_count: 1,
  pair_lock_add_pairs: 0,
  pair_lock_lock_first: true,
  pair_lock_recover_seconds: 20,
  pair_lock_skip_thin_bid: false,
  pair_lock_assets: AssetRegistry.getDefaultPathAssets(),
  cap_lock_enabled: false,
  cap_lock_max_loss_usd: 0.05,
  cap_lock_window_open_seconds: 90,
  cap_lock_allow_later: true,
  cap_lock_lot_count: 1,
  cap_lock_assets: AssetRegistry.getDefaultPathAssets(),
  buffer_run_enabled: false,
  buffer_run_ask_min_usd: 0.42,
  buffer_run_ask_max_usd: 0.62,
  buffer_run_take_usd: 0.12,
  buffer_run_stop_usd: 0.07,
  buffer_run_enter_elapsed_minutes: 3,
  buffer_run_enter_left_minutes: 5,
  buffer_run_flatten_minutes: 3,
  buffer_run_atr_mult: 1.25,
  buffer_run_min_gap_btc_usd: 40,
  buffer_run_min_gap_eth_usd: 2.5,
  buffer_run_fixed_dollars_per_trade: 2.5,
  buffer_run_pair_sum_skip: 0.98,
  buffer_run_skip_thin_bid: false,
  buffer_run_assets: ['BTC', 'ETH'],
  cheap_loop_enabled: false,
  cheap_loop_start_minutes: 2,
  cheap_loop_flatten_minutes: 5,
  cheap_loop_cheap_max_ask_usd: 0.4,
  cheap_loop_min_gap_usd: 0.1,
  cheap_loop_min_live_cushion_pct: 15,
  cheap_loop_take_usd: 0.05,
  cheap_loop_stop_enabled: false,
  cheap_loop_stop_usd: 0.06,
  cheap_loop_min_hold_minutes: 1,
  cheap_loop_cooldown_minutes: 2,
  cheap_loop_cycles: 1,
  cheap_loop_lot_count: 1,
  cheap_loop_skip_thin_bid: false,
  cheap_loop_assets: [],
  cheap_loop_hourly_enabled: false,
  cheap_loop_hourly_start_minutes: 10,
  cheap_loop_hourly_flatten_minutes: 5,
  cheap_loop_hourly_cheap_max_ask_usd: 0.4,
  cheap_loop_hourly_min_gap_usd: 0.1,
  cheap_loop_hourly_take_usd: 0.05,
  cheap_loop_hourly_stop_enabled: false,
  cheap_loop_hourly_stop_usd: 0.06,
  cheap_loop_hourly_min_hold_minutes: 2,
  cheap_loop_hourly_cooldown_minutes: 3,
  cheap_loop_hourly_cycles: 2,
  cheap_loop_hourly_lot_count: 1,
  cheap_loop_hourly_skip_thin_bid: false,
  cheap_loop_hourly_assets: [],
  cheap_loop_weekly_enabled: false,
  cheap_loop_weekly_start_minutes: 10,
  cheap_loop_weekly_flatten_minutes: 120,
  cheap_loop_weekly_cheap_max_ask_usd: 0.45,
  cheap_loop_weekly_min_gap_usd: 0.08,
  cheap_loop_weekly_take_usd: 0.08,
  cheap_loop_weekly_stop_enabled: true,
  cheap_loop_weekly_stop_usd: 0.12,
  cheap_loop_weekly_min_hold_minutes: 2,
  cheap_loop_weekly_cooldown_minutes: 3,
  cheap_loop_weekly_cycles: 10,
  cheap_loop_weekly_lot_count: 1,
  cheap_loop_weekly_skip_thin_bid: false,
  cheap_loop_weekly_assets: ['BTC', 'ETH'],
};

export type RiskFieldGroup = 'size' | 'caps' | 'timing';

export const RISK_GROUPS: { id: RiskFieldGroup; label: string }[] = [
  { id: 'size', label: 'Size' },
  { id: 'caps', label: 'Caps' },
  { id: 'timing', label: 'Timing & protect' },
];

export const RISK_FIELD_META: {
  key: keyof RiskConfig;
  label: string;
  group: RiskFieldGroup;
  kind: 'money' | 'int' | 'tif' | 'chase' | 'toggle' | 'ratio' | 'seconds' | 'percent';
  step: number;
  min: number;
  max: number;
}[] = [
  { key: 'fixed_dollars_per_trade', label: '$ per trade', group: 'size', kind: 'money', step: 1, min: 1, max: 500 },
  { key: 'max_dollars_per_trade', label: 'Max $ / trade', group: 'size', kind: 'money', step: 1, min: 1, max: 500 },
  { key: 'min_dollars_per_trade', label: 'Min $ / trade', group: 'size', kind: 'money', step: 1, min: 1, max: 500 },
  { key: 'max_open_positions', label: 'Max open positions', group: 'caps', kind: 'int', step: 1, min: 1, max: 50 },
  { key: 'max_trades_per_day', label: 'Max trades / day', group: 'caps', kind: 'int', step: 1, min: 1, max: 50000 },
  {
    key: 'max_trades_per_asset_per_window',
    label: 'Max trades / asset / 15m window',
    group: 'caps',
    kind: 'int',
    step: 1,
    min: 1,
    max: 5,
  },
  { key: 'daily_loss_stop_usd', label: 'Daily loss stop ($)', group: 'caps', kind: 'money', step: 5, min: 1, max: 10000 },
  { key: 'min_minutes_left', label: 'Min minutes left (Buy only)', group: 'timing', kind: 'int', step: 1, min: 0, max: 14 },
  {
    key: 'min_minutes_elapsed',
    label: 'Min minutes elapsed (Buy only)',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 0,
    max: 10,
  },
  { key: 'max_entry_ask_usd', label: 'Max entry ask ($) (Buy limit)', group: 'timing', kind: 'chase', step: 0.01, min: 0.5, max: 0.99 },
  { key: 'time_in_force', label: 'Time in force', group: 'timing', kind: 'tif', step: 0, min: 0, max: 0 },
  { key: 'chase_above_ask_usd', label: 'Slip (chase above ask)', group: 'timing', kind: 'chase', step: 0.01, min: 0, max: 0.05 },
  {
    key: 'cushion_lean_enter_mult',
    label: 'Enter when gap ≥ cushion ×',
    group: 'timing',
    kind: 'ratio',
    step: 0.05,
    min: 0.5,
    max: 1.5,
  },
  {
    key: 'cushion_lean_max_gap_mult',
    label: 'Skip if gap ≥ cushion ×',
    group: 'timing',
    kind: 'ratio',
    step: 0.25,
    min: 1.5,
    max: 5,
  },
  {
    key: 'smart_buy_enabled',
    label: 'Smart buy',
    group: 'timing',
    kind: 'toggle',
    step: 0,
    min: 0,
    max: 1,
  },
  {
    key: 'smart_buy_min_edge_usd',
    label: 'Min extra chance ($)',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.04,
    max: 0.15,
  },
  {
    key: 'protect_sell_enabled',
    label: 'Protect money (early sell)',
    group: 'timing',
    kind: 'toggle',
    step: 0,
    min: 0,
    max: 1,
  },
  {
    key: 'protect_sell_gap_ratio',
    label: 'Sell when gap ≥ cushion ×',
    group: 'timing',
    kind: 'ratio',
    step: 0.25,
    min: 0.5,
    max: 3,
  },
  {
    key: 'protect_sell_grace_seconds',
    label: 'Wait after fill before sell',
    group: 'timing',
    kind: 'seconds',
    step: 15,
    min: 0,
    max: 120,
  },
  {
    key: 'home_enter_cushion_mult',
    label: 'Enter when gap ≥ cushion ×',
    group: 'timing',
    kind: 'ratio',
    step: 0.05,
    min: 0.5,
    max: 1.5,
  },
  {
    key: 'home_sell_at_pct',
    label: 'Sell at',
    group: 'timing',
    kind: 'percent',
    step: 0.5,
    min: 0,
    max: 100,
  },
  {
    key: 'cushion_lean_sell_at_pct',
    label: 'Sell at',
    group: 'timing',
    kind: 'percent',
    step: 0.5,
    min: 0,
    max: 100,
  },
  {
    key: 'cash_out_enabled',
    label: 'Cash out',
    group: 'timing',
    kind: 'toggle',
    step: 0,
    min: 0,
    max: 1,
  },
  {
    key: 'cash_out_fixed_dollars_per_trade',
    label: '$ per trade',
    group: 'size',
    kind: 'money',
    step: 1,
    min: 1,
    max: 500,
  },
  {
    key: 'cash_out_max_dollars_per_trade',
    label: 'Max $ / trade',
    group: 'size',
    kind: 'money',
    step: 1,
    min: 1,
    max: 500,
  },
  {
    key: 'cash_out_min_dollars_per_trade',
    label: 'Min $ / trade',
    group: 'size',
    kind: 'money',
    step: 1,
    min: 1,
    max: 500,
  },
  {
    key: 'cash_out_enter_pct',
    label: 'Enter cushion (%)',
    group: 'timing',
    kind: 'int',
    step: 5,
    min: 40,
    max: 100,
  },
  {
    key: 'cash_out_max_ask_usd',
    label: 'Cash out max ask ($)',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.5,
    max: 0.99,
  },
  {
    key: 'cash_out_bid_usd',
    label: 'Cash out bid ($)',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.5,
    max: 0.99,
  },
  {
    key: 'cash_out_stop_usd',
    label: 'Cash out stop',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.03,
    max: 0.1,
  },
  {
    key: 'cash_out_skip_thin_bid',
    label: 'Skip thin bid',
    group: 'timing',
    kind: 'toggle',
    step: 0,
    min: 0,
    max: 1,
  },
  {
    key: 'gold_fade_enabled',
    label: 'Gold fade',
    group: 'timing',
    kind: 'toggle',
    step: 0,
    min: 0,
    max: 1,
  },
  {
    key: 'gold_fade_fixed_dollars_per_trade',
    label: '$ per trade',
    group: 'size',
    kind: 'money',
    step: 1,
    min: 1,
    max: 500,
  },
  {
    key: 'gold_fade_max_dollars_per_trade',
    label: 'Max $ / trade',
    group: 'size',
    kind: 'money',
    step: 1,
    min: 1,
    max: 500,
  },
  {
    key: 'gold_fade_min_dollars_per_trade',
    label: 'Min $ / trade',
    group: 'size',
    kind: 'money',
    step: 1,
    min: 1,
    max: 500,
  },
  {
    key: 'gold_fade_max_gap_usd',
    label: 'Max gap',
    group: 'timing',
    kind: 'money',
    step: 0.5,
    min: 1,
    max: 6,
  },
  {
    key: 'gold_fade_max_ask_usd',
    label: 'Max cheap ask',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.35,
    max: 0.55,
  },
  {
    key: 'gold_fade_take_usd',
    label: 'Take profit',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.05,
    max: 0.1,
  },
  {
    key: 'gold_fade_stop_usd',
    label: 'Gold fade stop',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.03,
    max: 0.1,
  },
  {
    key: 'gold_fade_flatten_minutes',
    label: 'Flatten with min left',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 2,
    max: 5,
  },
  {
    key: 'twap_lock_enabled',
    label: 'TWAP lock',
    group: 'timing',
    kind: 'toggle',
    step: 0,
    min: 0,
    max: 1,
  },
  {
    key: 'twap_lock_fixed_dollars_per_trade',
    label: '$ per trade',
    group: 'size',
    kind: 'money',
    step: 1,
    min: 1,
    max: 500,
  },
  {
    key: 'twap_lock_max_dollars_per_trade',
    label: 'Max $ / trade',
    group: 'size',
    kind: 'money',
    step: 1,
    min: 1,
    max: 500,
  },
  {
    key: 'twap_lock_min_dollars_per_trade',
    label: 'Min $ / trade',
    group: 'size',
    kind: 'money',
    step: 1,
    min: 1,
    max: 500,
  },
  {
    key: 'twap_lock_max_ask_usd',
    label: 'Max ask',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.9,
    max: 0.97,
  },
  {
    key: 'last_minute_enabled',
    label: 'Last-minute',
    group: 'timing',
    kind: 'toggle',
    step: 0,
    min: 0,
    max: 1,
  },
  {
    key: 'last_minute_max_ask_usd',
    label: 'Entry ask',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.8,
    max: 0.99,
  },
  {
    key: 'last_minute_watch_seconds',
    label: 'Watch start (s left)',
    group: 'timing',
    kind: 'seconds',
    step: 5,
    min: 70,
    max: 180,
  },
  {
    key: 'last_minute_enter_seconds',
    label: 'First clip by (s left)',
    group: 'timing',
    kind: 'seconds',
    step: 5,
    min: 30,
    max: 150,
  },
  {
    key: 'last_minute_stop_seconds',
    label: 'Stop with (s left)',
    group: 'timing',
    kind: 'seconds',
    step: 1,
    min: 5,
    max: 20,
  },
  {
    key: 'last_minute_ladder_seconds',
    label: 'Ladder wait (s)',
    group: 'timing',
    kind: 'seconds',
    step: 1,
    min: 1,
    max: 5,
  },
  {
    key: 'last_minute_clip_count',
    label: 'Clip contracts/asset',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 5,
  },
  {
    key: 'last_minute_max_clips',
    label: 'Max clips/asset',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 20,
  },
  {
    key: 'last_minute_both_min_ask',
    label: 'Both min favorite',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.7,
    max: 0.95,
  },
  {
    key: 'last_minute_both_gap',
    label: 'Both min gap',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.05,
    max: 0.2,
  },
  {
    key: 'last_minute_flip_sell_usd',
    label: 'Sell if flip ≥',
    group: 'timing',
    kind: 'chase',
    step: 0.05,
    min: 0,
    max: 0.2,
  },
  {
    key: 'last_minute_atr_cushion_enabled',
    label: 'Late ATR cushion',
    group: 'timing',
    kind: 'toggle',
    step: 0,
    min: 0,
    max: 1,
  },
  {
    key: 'last_minute_atr_ask_usd',
    label: 'High ask floor',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.8,
    max: 0.95,
  },
  {
    key: 'last_minute_atr_mult',
    label: 'Lead ≥ ATR ×',
    group: 'timing',
    kind: 'ratio',
    step: 0.05,
    min: 1,
    max: 1.5,
  },
  { key: 'step_buy_enabled', label: 'Step buy', group: 'timing', kind: 'toggle', step: 1, min: 0, max: 1 },
  {
    key: 'step_buy_start_minutes',
    label: 'Start after',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 2,
    max: 10,
  },
  {
    key: 'step_buy_cushion_pct',
    label: 'Cushion %',
    group: 'timing',
    kind: 'int',
    step: 5,
    min: 25,
    max: 100,
  },
  {
    key: 'step_buy_add_cushion_pct',
    label: 'Add cushion %',
    group: 'timing',
    kind: 'int',
    step: 5,
    min: 25,
    max: 100,
  },
  {
    key: 'step_buy_lot_count',
    label: 'Lot contracts',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 5,
  },
  {
    key: 'step_buy_add_wait_minutes',
    label: 'Add wait',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 3,
  },
  {
    key: 'step_buy_add_band_usd',
    label: 'Add band',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0,
    max: 0.1,
  },
  {
    key: 'step_buy_max_lots',
    label: 'Max lots',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 8,
  },
  {
    key: 'step_buy_stop_usd',
    label: 'Stop',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.01,
    max: 0.1,
  },
  {
    key: 'step_buy_max_ask_usd',
    label: 'Entry ask',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.5,
    max: 0.9,
  },
  { key: 'spike_fade_enabled', label: 'Spike fade', group: 'timing', kind: 'toggle', step: 1, min: 0, max: 1 },
  {
    key: 'spike_fade_start_minutes',
    label: 'Start after',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 4,
  },
  {
    key: 'spike_fade_until_minutes',
    label: 'Until minute',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 4,
    max: 8,
  },
  {
    key: 'spike_fade_expensive_min_usd',
    label: 'Expensive min',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.7,
    max: 0.85,
  },
  {
    key: 'spike_fade_expensive_max_usd',
    label: 'Expensive max',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.75,
    max: 0.9,
  },
  {
    key: 'spike_fade_cheap_min_usd',
    label: 'Cheap min',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.15,
    max: 0.3,
  },
  {
    key: 'spike_fade_cheap_max_usd',
    label: 'Cheap max',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.2,
    max: 0.35,
  },
  {
    key: 'spike_fade_take_ask_usd',
    label: 'Take ask',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.35,
    max: 0.5,
  },
  {
    key: 'spike_fade_stop_ask_usd',
    label: 'Stop ask',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.05,
    max: 0.15,
  },
  {
    key: 'spike_fade_flatten_minutes',
    label: 'Flatten left',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 2,
    max: 5,
  },
  {
    key: 'spike_fade_lot_count',
    label: 'Lot contracts',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 5,
  },
  { key: 'pair_lock_enabled', label: 'Pair lock', group: 'timing', kind: 'toggle', step: 1, min: 0, max: 1 },
  {
    key: 'pair_lock_start_minutes',
    label: 'Start after',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 8,
  },
  {
    key: 'pair_lock_until_minutes',
    label: 'Until minute',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 6,
    max: 14,
  },
  {
    key: 'pair_lock_runner_max_ask_usd',
    label: 'Runner max ask',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.4,
    max: 0.7,
  },
  {
    key: 'pair_lock_min_lock_usd',
    label: 'Min lock',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.02,
    max: 0.15,
  },
  {
    key: 'pair_lock_flatten_minutes',
    label: 'Flatten unmatched',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 2,
    max: 5,
  },
  {
    key: 'pair_lock_recover_seconds',
    label: 'Recover wait',
    group: 'timing',
    kind: 'int',
    step: 5,
    min: 10,
    max: 60,
  },
  {
    key: 'pair_lock_runner_stop_usd',
    label: 'Runner stop',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0,
    max: 0.2,
  },
  {
    key: 'pair_lock_lot_count',
    label: 'Lot contracts',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 5,
  },
  {
    key: 'pair_lock_add_pairs',
    label: 'Add new pair',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 0,
    max: 3,
  },
  { key: 'cap_lock_enabled', label: 'Cap lock', group: 'timing', kind: 'toggle', step: 1, min: 0, max: 1 },
  {
    key: 'cap_lock_max_loss_usd',
    label: 'Max lock loss',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.01,
    max: 0.08,
  },
  {
    key: 'cap_lock_window_open_seconds',
    label: 'Try first',
    group: 'timing',
    kind: 'int',
    step: 10,
    min: 30,
    max: 180,
  },
  {
    key: 'cap_lock_lot_count',
    label: 'Lot contracts',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 5,
  },
  { key: 'buffer_run_enabled', label: 'Buffer run', group: 'timing', kind: 'toggle', step: 1, min: 0, max: 1 },
  {
    key: 'buffer_run_ask_min_usd',
    label: 'Ask min',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.35,
    max: 0.55,
  },
  {
    key: 'buffer_run_ask_max_usd',
    label: 'Ask max',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.5,
    max: 0.75,
  },
  {
    key: 'buffer_run_take_usd',
    label: 'Take',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.06,
    max: 0.2,
  },
  {
    key: 'buffer_run_stop_usd',
    label: 'Stop',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.04,
    max: 0.15,
  },
  {
    key: 'buffer_run_sell_at_pct',
    label: 'Sell at',
    group: 'timing',
    kind: 'percent',
    step: 0.5,
    min: 0,
    max: 100,
  },
  {
    key: 'buffer_run_enter_elapsed_minutes',
    label: 'Enter after',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 8,
  },
  {
    key: 'buffer_run_enter_left_minutes',
    label: 'Enter left',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 4,
    max: 10,
  },
  {
    key: 'buffer_run_flatten_minutes',
    label: 'Flatten left',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 2,
    max: 5,
  },
  {
    key: 'buffer_run_atr_mult',
    label: 'ATR ×',
    group: 'timing',
    kind: 'ratio',
    step: 0.05,
    min: 1,
    max: 1.5,
  },
  {
    key: 'buffer_run_min_gap_btc_usd',
    label: 'BTC min gap $',
    group: 'timing',
    kind: 'money',
    step: 1,
    min: 10,
    max: 200,
  },
  {
    key: 'buffer_run_min_gap_eth_usd',
    label: 'ETH min gap $',
    group: 'timing',
    kind: 'money',
    step: 0.1,
    min: 0.5,
    max: 20,
  },
  {
    key: 'buffer_run_fixed_dollars_per_trade',
    label: '$ per trade',
    group: 'timing',
    kind: 'money',
    step: 0.5,
    min: 1,
    max: 50,
  },
  {
    key: 'buffer_run_pair_sum_skip',
    label: 'Pair-sum skip',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.95,
    max: 1,
  },
  { key: 'cheap_loop_enabled', label: '15 min', group: 'timing', kind: 'toggle', step: 1, min: 0, max: 1 },
  {
    key: 'cheap_loop_start_minutes',
    label: 'Start after',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 20,
  },
  {
    key: 'cheap_loop_flatten_minutes',
    label: 'Flatten left',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 3,
    max: 10,
  },
  {
    key: 'cheap_loop_cheap_max_ask_usd',
    label: 'Cheap max ask',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.25,
    max: 0.45,
  },
  {
    key: 'cheap_loop_min_gap_usd',
    label: 'Min gap',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.08,
    max: 0.2,
  },
  {
    key: 'cheap_loop_min_live_cushion_pct',
    label: 'Min live %',
    group: 'timing',
    kind: 'int',
    step: 5,
    min: 0,
    max: 50,
  },
  {
    key: 'cheap_loop_take_usd',
    label: 'Take',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.03,
    max: 0.08,
  },
  {
    key: 'cheap_loop_stop_usd',
    label: 'Stop',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.05,
    max: 0.12,
  },
  {
    key: 'cheap_loop_min_hold_minutes',
    label: 'Min hold',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 8,
  },
  {
    key: 'cheap_loop_cooldown_minutes',
    label: 'Cooldown',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 10,
  },
  {
    key: 'cheap_loop_cycles',
    label: 'Cycles',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 5,
  },
  {
    key: 'cheap_loop_lot_count',
    label: 'Lot contracts',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 5,
  },
  { key: 'cheap_loop_hourly_enabled', label: 'Hourly', group: 'timing', kind: 'toggle', step: 1, min: 0, max: 1 },
  {
    key: 'cheap_loop_hourly_start_minutes',
    label: 'Start after',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 20,
  },
  {
    key: 'cheap_loop_hourly_flatten_minutes',
    label: 'Flatten left',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 3,
    max: 10,
  },
  {
    key: 'cheap_loop_hourly_cheap_max_ask_usd',
    label: 'Cheap max ask',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.25,
    max: 0.45,
  },
  {
    key: 'cheap_loop_hourly_min_gap_usd',
    label: 'Min gap',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.08,
    max: 0.2,
  },
  {
    key: 'cheap_loop_hourly_take_usd',
    label: 'Take',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.03,
    max: 0.08,
  },
  {
    key: 'cheap_loop_hourly_stop_usd',
    label: 'Stop',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.05,
    max: 0.12,
  },
  {
    key: 'cheap_loop_hourly_min_hold_minutes',
    label: 'Min hold',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 8,
  },
  {
    key: 'cheap_loop_hourly_cooldown_minutes',
    label: 'Cooldown',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 10,
  },
  {
    key: 'cheap_loop_hourly_cycles',
    label: 'Cycles',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 10,
  },
  {
    key: 'cheap_loop_hourly_lot_count',
    label: 'Lot contracts',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 5,
  },
  { key: 'cheap_loop_weekly_enabled', label: 'Weekly', group: 'timing', kind: 'toggle', step: 1, min: 0, max: 1 },
  {
    key: 'cheap_loop_weekly_start_minutes',
    label: 'Start after',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 20,
  },
  {
    key: 'cheap_loop_weekly_flatten_minutes',
    label: 'Flatten left',
    group: 'timing',
    kind: 'int',
    step: 30,
    min: 30,
    max: 720,
  },
  {
    key: 'cheap_loop_weekly_cheap_max_ask_usd',
    label: 'Cheap max ask',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.25,
    max: 0.45,
  },
  {
    key: 'cheap_loop_weekly_min_gap_usd',
    label: 'Min gap',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.08,
    max: 0.2,
  },
  {
    key: 'cheap_loop_weekly_take_usd',
    label: 'Take',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.03,
    max: 0.08,
  },
  {
    key: 'cheap_loop_weekly_stop_usd',
    label: 'Stop',
    group: 'timing',
    kind: 'chase',
    step: 0.01,
    min: 0.05,
    max: 0.12,
  },
  {
    key: 'cheap_loop_weekly_min_hold_minutes',
    label: 'Min hold',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 8,
  },
  {
    key: 'cheap_loop_weekly_cooldown_minutes',
    label: 'Cooldown',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 10,
  },
  {
    key: 'cheap_loop_weekly_cycles',
    label: 'Cycles',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 50,
  },
  {
    key: 'cheap_loop_weekly_lot_count',
    label: 'Lot contracts',
    group: 'timing',
    kind: 'int',
    step: 1,
    min: 1,
    max: 5,
  },
];

export const TIME_IN_FORCE_OPTIONS: { value: TimeInForce; label: string }[] = [
  { value: 'immediate_or_cancel', label: 'IOC' },
  { value: 'good_till_canceled', label: 'GTC' },
  { value: 'fill_or_kill', label: 'FOK' },
];

export const SHARED_RISK_FIELD_KEYS: (keyof RiskConfig)[] = [
  'max_open_positions',
  'max_trades_per_day',
  'max_trades_per_asset_per_window',
  'daily_loss_stop_usd',
];

export const PATH_RISK_FIELD_KEYS: (keyof RiskConfig)[] = [
  'fixed_dollars_per_trade',
  'max_dollars_per_trade',
  'min_dollars_per_trade',
  'min_minutes_left',
  'min_minutes_elapsed',
  'max_entry_ask_usd',
  'time_in_force',
  'chase_above_ask_usd',
];

export const CUSHION_LEAN_ENTER_FIELD_KEYS: (keyof RiskConfig)[] = ['cushion_lean_enter_mult'];

export const CUSHION_LEAN_MAX_GAP_FIELD_KEYS: (keyof RiskConfig)[] = [
  'cushion_lean_max_gap_mult',
];

/** Home Buy enter × cushion. Restore with Home Buy tab. */
export const HOME_ENTER_CUSHION_FIELD_KEYS: (keyof RiskConfig)[] = ['home_enter_cushion_mult'];

/** Home Buy take-profit %. Restore with Home Buy tab. */
export const HOME_SELL_AT_RISK_FIELD_KEYS: (keyof RiskConfig)[] = ['home_sell_at_pct'];

/** Cushion lean take-profit %. Restore with Auto-trade / Cushion lean. */
export const CUSHION_LEAN_SELL_AT_RISK_FIELD_KEYS: (keyof RiskConfig)[] = [
  'cushion_lean_sell_at_pct',
];

export const SMART_BUY_RISK_FIELD_KEYS: (keyof RiskConfig)[] = [
  'smart_buy_enabled',
  'smart_buy_min_edge_usd',
];

export const PROTECT_RISK_FIELD_KEYS: (keyof RiskConfig)[] = [
  'protect_sell_enabled',
  'protect_sell_gap_ratio',
  'protect_sell_grace_seconds',
];

export const CASH_OUT_RISK_FIELD_KEYS: (keyof RiskConfig)[] = [
  'cash_out_enabled',
  'cash_out_fixed_dollars_per_trade',
  'cash_out_max_dollars_per_trade',
  'cash_out_min_dollars_per_trade',
  'cash_out_enter_pct',
  'cash_out_max_ask_usd',
  'cash_out_bid_usd',
  'cash_out_stop_usd',
  'cash_out_skip_thin_bid',
];

export const GOLD_FADE_RISK_FIELD_KEYS: (keyof RiskConfig)[] = [
  'gold_fade_enabled',
  'gold_fade_fixed_dollars_per_trade',
  'gold_fade_max_dollars_per_trade',
  'gold_fade_min_dollars_per_trade',
  'gold_fade_max_gap_usd',
  'gold_fade_max_ask_usd',
  'gold_fade_take_usd',
  'gold_fade_stop_usd',
  'gold_fade_flatten_minutes',
];

export const TWAP_LOCK_RISK_FIELD_KEYS: (keyof RiskConfig)[] = [
  'twap_lock_enabled',
  'twap_lock_fixed_dollars_per_trade',
  'twap_lock_max_dollars_per_trade',
  'twap_lock_min_dollars_per_trade',
  'twap_lock_max_ask_usd',
];

export const LAST_MINUTE_RISK_FIELD_KEYS: (keyof RiskConfig)[] = [
  'last_minute_enabled',
  'last_minute_max_ask_usd',
  'last_minute_watch_seconds',
  'last_minute_enter_seconds',
  'last_minute_stop_seconds',
  'last_minute_ladder_seconds',
  'last_minute_clip_count',
  'last_minute_max_clips',
  'last_minute_both_min_ask',
  'last_minute_both_gap',
  'last_minute_flip_sell_usd',
  'last_minute_atr_cushion_enabled',
  'last_minute_atr_ask_usd',
  'last_minute_atr_mult',
];

export const STEP_BUY_RISK_FIELD_KEYS: (keyof RiskConfig)[] = [
  'step_buy_enabled',
  'step_buy_start_minutes',
  'step_buy_cushion_pct',
  'step_buy_add_cushion_pct',
  'step_buy_lot_count',
  'step_buy_add_wait_minutes',
  'step_buy_add_band_usd',
  'step_buy_max_lots',
  'step_buy_stop_usd',
  'step_buy_max_ask_usd',
];

export const SPIKE_FADE_RISK_FIELD_KEYS: (keyof RiskConfig)[] = [
  'spike_fade_enabled',
  'spike_fade_start_minutes',
  'spike_fade_until_minutes',
  'spike_fade_expensive_min_usd',
  'spike_fade_expensive_max_usd',
  'spike_fade_cheap_min_usd',
  'spike_fade_cheap_max_usd',
  'spike_fade_take_ask_usd',
  'spike_fade_stop_ask_usd',
  'spike_fade_flatten_minutes',
  'spike_fade_lot_count',
];

export const PAIR_LOCK_RISK_FIELD_KEYS: (keyof RiskConfig)[] = [
  'pair_lock_enabled',
  'pair_lock_start_minutes',
  'pair_lock_until_minutes',
  'pair_lock_runner_max_ask_usd',
  'pair_lock_min_lock_usd',
  'pair_lock_flatten_minutes',
  'pair_lock_recover_seconds',
  'pair_lock_runner_stop_usd',
  'pair_lock_lot_count',
  'pair_lock_add_pairs',
];

export const CAP_LOCK_RISK_FIELD_KEYS: (keyof RiskConfig)[] = [
  'cap_lock_enabled',
  'cap_lock_max_loss_usd',
  'cap_lock_window_open_seconds',
  'cap_lock_lot_count',
];

export const BUFFER_RUN_RISK_FIELD_KEYS: (keyof RiskConfig)[] = [
  'buffer_run_enabled',
  'buffer_run_ask_min_usd',
  'buffer_run_ask_max_usd',
  'buffer_run_take_usd',
  'buffer_run_stop_usd',
  'buffer_run_sell_at_pct',
  'buffer_run_enter_elapsed_minutes',
  'buffer_run_enter_left_minutes',
  'buffer_run_flatten_minutes',
  'buffer_run_atr_mult',
  'buffer_run_min_gap_btc_usd',
  'buffer_run_min_gap_eth_usd',
  'buffer_run_fixed_dollars_per_trade',
  'buffer_run_pair_sum_skip',
];

export const CHEAP_LOOP_RISK_FIELD_KEYS: (keyof RiskConfig)[] = [
  'cheap_loop_enabled',
  'cheap_loop_start_minutes',
  'cheap_loop_flatten_minutes',
  'cheap_loop_cheap_max_ask_usd',
  'cheap_loop_min_gap_usd',
  'cheap_loop_min_live_cushion_pct',
  'cheap_loop_take_usd',
  'cheap_loop_min_hold_minutes',
  'cheap_loop_cooldown_minutes',
  'cheap_loop_cycles',
  'cheap_loop_lot_count',
];

export const CHEAP_LOOP_HOURLY_RISK_FIELD_KEYS: (keyof RiskConfig)[] = [
  'cheap_loop_hourly_enabled',
  'cheap_loop_hourly_start_minutes',
  'cheap_loop_hourly_flatten_minutes',
  'cheap_loop_hourly_cheap_max_ask_usd',
  'cheap_loop_hourly_min_gap_usd',
  'cheap_loop_hourly_take_usd',
  'cheap_loop_hourly_min_hold_minutes',
  'cheap_loop_hourly_cooldown_minutes',
  'cheap_loop_hourly_cycles',
  'cheap_loop_hourly_lot_count',
];

export const CHEAP_LOOP_WEEKLY_RISK_FIELD_KEYS: (keyof RiskConfig)[] = [
  'cheap_loop_weekly_enabled',
  'cheap_loop_weekly_start_minutes',
  'cheap_loop_weekly_flatten_minutes',
  'cheap_loop_weekly_cheap_max_ask_usd',
  'cheap_loop_weekly_min_gap_usd',
  'cheap_loop_weekly_take_usd',
  'cheap_loop_weekly_min_hold_minutes',
  'cheap_loop_weekly_cooldown_minutes',
  'cheap_loop_weekly_cycles',
  'cheap_loop_weekly_lot_count',
];

export const AUTO_ONLY_RISK_FIELD_KEYS: (keyof RiskConfig)[] = [
  ...CUSHION_LEAN_SELL_AT_RISK_FIELD_KEYS,
  ...CUSHION_LEAN_ENTER_FIELD_KEYS,
  ...CUSHION_LEAN_MAX_GAP_FIELD_KEYS,
  ...SMART_BUY_RISK_FIELD_KEYS,
  ...PROTECT_RISK_FIELD_KEYS,
  ...CASH_OUT_RISK_FIELD_KEYS,
  ...GOLD_FADE_RISK_FIELD_KEYS,
  ...TWAP_LOCK_RISK_FIELD_KEYS,
  ...LAST_MINUTE_RISK_FIELD_KEYS,
  ...STEP_BUY_RISK_FIELD_KEYS,
  ...SPIKE_FADE_RISK_FIELD_KEYS,
  ...PAIR_LOCK_RISK_FIELD_KEYS,
  ...CAP_LOCK_RISK_FIELD_KEYS,
  ...BUFFER_RUN_RISK_FIELD_KEYS,
  ...CHEAP_LOOP_RISK_FIELD_KEYS,
  ...CHEAP_LOOP_HOURLY_RISK_FIELD_KEYS,
  ...CHEAP_LOOP_WEEKLY_RISK_FIELD_KEYS,
];

export function cloneDefaultRisk(): RiskConfig {
  return { ...DEFAULT_RISK_CONFIG };
}
