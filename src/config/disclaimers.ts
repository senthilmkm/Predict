...
export const DISCLAIMER_VERSION = '2026-09-10.2';
...
export const DISCLAIMER_LONG = [
  ...
  'Home Buy / Sell: one tap tells Cloud Run to place now. The phone never talks to Kalshi. Auto-trade Off does not block a tap. Home Buy uses Settings → Risk → Home Buy (size, minutes left/elapsed, max entry ask, time in force, chase). Shared limits (max open, trades/day, trades per 15m window, daily loss stop), cushions, and asset on/off apply to both Home Buy and Auto-trade. Auto-trade uses the Auto-trade tab, including Protect money. Chase can make you pay a little more than the visible ask, still capped by that path’s max entry ask (max $0.99). GTC can leave a resting order on Kalshi until fill or cancel. IOC / FOK can miss and you get no position. You can lose the full notional if the contract settles against you. Kill Switch and the Last signals Buy / Sell flag hide and disable these taps.',
