...
export type TradeStreamEntryLabel = 'Home' | 'Auto' | 'Cash out' | '—';
...
export function parseTradeStreamQuery(query: Record<string, unknown> | undefined): TradeStreamFilters & {
  ...
  const pathRaw = parseTradeEntryPath(q.entryPath ?? q.path);
...
export function tradeMatchesFilters(t: TradeRecordDoc, f: TradeStreamFilters): boolean {
  ...
  if (f.entryPath && parseTradeEntryPath(t.entryPath) !== f.entryPath) return false;
  if (f.userId) {
