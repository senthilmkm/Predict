...
export type PairLockPlaceFn = (input: {
  ticker: string;
  side: 'bid' | 'ask';
  count: string;
  price: string;
...
export function pendingPairLockRunnerTradesForMarket(
...
export async function runCloudPairLockFlatten(opts: {
  userId: string;
  asset: string;
  ticker: string;
  lean: {
  ...
  const held = pendingPairLockRunnerTradesForMarket(opts.trades, opts.ticker);
  for (const trade of held) {
    ...
    const order = buildPairLockSellOrder({
    ...
    const clientOrderId = `plk-${trade.tradeId}`.slice(0, 64);
    ...
    try {
      ...
      await revertPairLockClaim(opts.userId, trade);
